...
void place_ships(Board *board) {
    // 1 Carrier (5), 1 Battleship (4), 2 Cruisers (3), 2 Destroyers (2), 3 Submarines (1)
    int ship_index = 0;
    // Carrier
    if (!place_ship(board, "carrier", 5, ship_index)) { perror("Error placing carrier"); }
    ship_index++;
    // Battleship
    if (!place_ship(board, "battleship", 4, ship_index)) { perror("Error placing battleship"); }
    ship_index++;
    // 2 Cruisers
    if (!place_ship(board, "cruiser", 3, ship_index)) { perror("Error placing cruiser1"); }
    ship_index++;
    if (!place_ship(board, "cruiser", 3, ship_index)) { perror("Error placing cruiser2"); }
    ship_index++;
    // 2 Destroyers
    if (!place_ship(board, "destroyer", 2, ship_index)) { perror("Error placing destroyer1"); }
    ship_index++;
    if (!place_ship(board, "destroyer", 2, ship_index)) { perror("Error placing destroyer2"); }
    ship_index++;
    // 3 Submarines
    if (!place_ship(board, "submarine", 1, ship_index)) { perror("Error placing submarine1"); }
    ship_index++;
    if (!place_ship(board, "submarine", 1, ship_index)) { perror("Error placing submarine2"); }
    ship_index++;
    if (!place_ship(board, "submarine", 1, ship_index)) { perror("Error placing submarine3"); }
    ship_index++;
}
...
void get_ship_data(Board *board, char *buffer, size_t buffer_size) {
    char temp[256] = "";
    int counts[26] = {0}; // This is to count how many ships of each type, using the first letter of the name as a key
    for (int i = 0; i < board->ship_count; i++) {
